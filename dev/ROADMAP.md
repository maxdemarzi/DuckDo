...
6. ~~**Causal discovery** — `do_discover()` proposing a DAG from data.~~ **DONE**, under the two conditions this item set. It was deliberately last: it is the feature users most want and the one most likely to produce confident nonsense.
   ...
   **Re-landing was attempted and abandoned; do not try it again this way.** The third defect is real and was fixed - the two sides of the independence test were built from the same weights, and correlated feature maps cancel the cross-covariance the statistic is made of - along with a wider basis (ten features per side, from five) and more rows (8,000, from 2,000, since the test is linear in rows and quartic in features). A fourth flaw was found and fixed too: pairs were settled in column order, so a confounded pair could be decided before the obvious parent of one of its members was known, with that parent's effect still in the residual diluting the very dependence the test looks for. Settling the strongest pairs first took the hardest case from 8 of 20 to 19 of 20.

   With all four fixed it measured 18-20 of 20 across every confounder strength, with no false alarms. Then the order in which two unrelated arrays are drawn from the same engine was changed - nothing else, not one line of the algorithm - and the same study on the same data gave 0-3 of 20, with a false alarm appearing. The power of the test is dominated by which ten weights happen to be drawn.

   That is the finding, and it is not a tuning problem. An affordable basis is not robust and a robust basis is not affordable: the reference needs twenty features per side for a stable result, which puts the null covariance at 400 by 400 and a three-variable problem at twenty seconds. Anything built on a single fixed feature draw will keep producing measurements that look like a working method and are not, which is what happened three times here. Re-landing needs a different statistic - averaging over draws, or a moment-based independence test with no draws in it at all - not a bigger constant. An independent reference implementation of the same pairwise test recovers the hidden cause 23 of 40 times at five features per side and 39 of 40 at twenty, where the C++ managed none - because `IndependenceP` builds both sides' features from the *same* five weights, differing only by each vector's bandwidth, instead of drawing each side its own. Correlated feature maps on the two sides is exactly what cancels the cross-covariance the statistic is made of.

   The cost is the other half of why. Seven variables over 20,000 rows took 5.5 seconds with `bootstrap := 0` and 25 with `bootstrap := 10`, so the default fifty would be minutes - for a method whose headline claim did not hold.
