...
struct KernelTest {
	...
	vector<double> phase;
	//! Weights and phases for features of a vector that is not one of the columns - the
	//! residual RESIT scores a candidate sink by. These used to be borrowed from the
	//! first column's slice of `weight`, which made their spread a matter of luck rather
	//! than of drawing.
	vector<double> loose_weight;
	vector<double> loose_phase;
...
double KernelTest::ResidualDependence(const vector<double> &residual, const vector<double> &fz) const {
	...
	const double scale = std::sqrt(2.0 / static_cast<double>(d));
	for (idx_t c = 0; c < d; c++) {
		const double w = loose_weight[c] / bandwidth;
		const double b = loose_phase[c];
		for (idx_t r = 0; r < m; r++) {
...
KernelTest MakeKernelTest(const NumericTable &t, const vector<idx_t> &rows, int64_t seed) {
	...
	// These features decide the graph, not just how stable it looks, so they have to be
	// the same everywhere. std::normal_distribution and std::uniform_real_distribution
	// are implementation-defined: the same engine and seed give different draws under
	// libstdc++ than under MSVC. Drawing from the engine's bits and inverting the normal
	// CDF is fixed by arithmetic instead.
	...
	for (idx_t c = 0; c < kSetFeatures; c++) {
		for (idx_t j = 0; j < t.p; j++) {
			k.weight[c * t.p + j] = gaussian();
		}
		k.phase[c] = 2.0 * 3.14159265358979323846 * unit();
	}

	k.loose_weight.resize(kXyFeatures);
	k.loose_phase.resize(kXyFeatures);
	for (idx_t c = 0; c < kXyFeatures; c++) {
		k.loose_weight[c] = gaussian();
		k.loose_phase[c] = 2.0 * 3.14159265358979323846 * unit();
	}
	...
	for (idx_t j = 0; j < t.p; j++) {
		...
		for (idx_t c = 0; c < kXyFeatures; c++) {
			const double w = gaussian() / bandwidth;
			const double b = 2.0 * 3.14159265358979323846 * unit();
			for (idx_t r = 0; r < k.m; r++) {
