...
|---|---|---|---|
| [A/B test with non-compliance](#1-an-ab-test-nobody-complied-with) | 0.1761 | **0.1236** | 0.12 |
| [Observational pricing](#2-pricing-where-the-sign-is-wrong) | **+1.50** | **−2.5175** | −2.5 |
| [Churn intervention](#3-a-retention-call-worth-targeting) | −0.0248 | **−0.0909** | −0.0896 |
...
```
uptake in control                       0.202
uptake in treatment                     0.700
per-protocol (completed vs not)         0.1761
intention-to-treat (assigned vs not)    0.0615
```

Two wrong answers and no obvious way to choose between them. **Per-protocol** (0.1761)
compares compliers to non-compliers and inherits every difference between those groups.
**Intention-to-treat** (0.0615) is unbiased but answers a different question: it is the
...
──────  ──────  ──────  ─────────────  ───────────────
0.1236  0.1023  0.1450         9989.6  false
```

**0.1236**, and the interval covers 0.12.

Two things to check whenever you do this. The **first-stage F** at 9,990 says the
...
in uptake = 0.1236 × 0.498 = 0.0616, against an ITT of 0.0615. That identity is a free
check that nothing is wired up backwards.
...
```sql
SELECT round(regr_slope(units, price),3) AS naive_slope FROM sales;   -- +1.504
```

**+1.504.** A model fitted on this would recommend raising prices to sell more. This is
...
**−2.5175** against a truth of −2.5. Right magnitude, right sign.
...
──────  ──────────────  ─────  ─────  ───────────────
  6.18           108.9  108.0  109.9             1250
 19.29            76.3   76.1   76.5             2521
 22.24            68.9   68.8   68.9             2519
 25.15            61.5   61.4   61.7             2516
 39.13            26.0   25.0   26.9             1257
```

The grid is placed on observed quantiles, so it never extrapolates past prices you have
actually charged. `n_within_decile` is how much data sits near each point — about 1,250 at
the ends against about 2,520 in the middle — which is why the intervals fan out at the extremes.
...
do_ate                   -0.0909   [-0.1012, -0.0806]
```

The naive comparison makes the call look **3.7× weaker** than it is.
...
engagement > 0.5021     14997       0.0051  0.05  do not treat
```
...
─────  ──────────  ─────────────  ───────────  ──────
 0.20        6000         0.0375       0.0182  0.0194
 0.40       12000         0.0709       0.0363  0.0346
 0.50       15000         0.0852       0.0454  0.0398
 0.60       18000         0.0897       0.0545  0.0352
 0.80       24000         0.0926       0.0726  0.0199
 1.00       30000         0.0908       0.0908  0.0000
```

Calling the right **half** captures 0.0852 of the 0.0908 available from calling
