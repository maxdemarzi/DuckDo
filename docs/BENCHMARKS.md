...
**6× slower than the same model in Python** on eight thousand rows, for a reason
that is understood and fixable.
...
`aipw` lands within 0.14 of the average effect while getting individual effects
wrong by 2.2.
...
|---|---|---|---|---|
| **DuckDo `causalpfn`** | **0.078** | **0.070** | **0.416** | 2.31 |
| CausalPFN (python) | 0.082 | 0.075 | 0.423 | 1.79 |
| DoWhy PSW | 0.125 | 0.096 | — | 17.63 |
| DuckDo `aipw` | 0.137 | 0.108 | 2.226 | **0.19** |
| EconML `LinearDML` | 0.147 | 0.116 | 2.161 | 3.36 |
| EconML `LinearDRLearner` | 0.259 | 0.166 | 3.131 | 3.34 |
| EconML `CausalForestDML` | 0.390 | 0.066 | 2.749 | 5.48 |
| DuckDo `dml` | 0.786 | 0.160 | 2.226 | 0.19 |
...
**`dml`'s mean is a single replication.** 0.786 mean against a 0.160 median: on
replication 9, whose true effect is 10.47 where every other replication sits near
4, `dml` returns 4.57 and misses by **5.9**. Partialling out a treatment this
strongly predicted, with a linear first stage, on 747 rows, is where that estimator
breaks. `aipw` on the same replication returns 10.13. **Use the median column**;
...
|---|---|---|---|---|
| DuckDo `aipw` | **0.009** | 0.073 | 0.020 | 0.992 |
| DuckDo `dml` | 0.006 | 0.073 | 0.057 | 0.992 |
| DuckDo `causalpfn` | 0.043 | 0.096 | 0.032 | 0.451 |
...
**The average effect barely moves.** Every method lands within 0.07 of the truth on
...
`aipw` posts a PEHE of **0.992** against `CausalForestDML`'s **0.213**. DuckDo's
...
| DuckDo `aipw` | 1629.1 | 165.2 |
| DoWhy PSW | 1559.6 | 234.7 |
...
3.9 dollars is a good result and partly luck — its 90-dollar advantage over
`dml` is a quarter of what separates the best and worst methods on a sample this
small.
...
| DuckDo `causalpfn` | 2.31 | 148.5 |
| DuckDo `causalpfn`, `duckdo_query_chunk=8192` | — | **20.9** |
| CausalPFN (python, cpu) | 1.79 | 22.5 |
| CausalPFN (python, gpu) | 1.69 | 12.0 |
| EconML `LinearDML` | 3.36 | 3.30 |
| EconML `CausalForestDML` | 5.48 | 15.4 |
| DoWhy PSW | 17.63 | 3.01 |
...
small data — which is why DuckDo beats it by 17× on 747 rows and 15× on 8,000.

### The CFM path is 6× slower than it should be

148 seconds against the reference package's 22, on the same weights and the same
...
|---|---|---|---|
| 512 (default) | 128.0 | 2,063 | 2.9502346122190355 |
| 2048 | 42.5 | 3,334 | 2.9502346122190355 |
| 4096 | 29.3 | 3,593 | 2.9502346122190355 |
| 8192 | 21.5 | 5,347 | 2.9502346122190355 |

**The estimate is bit-identical at every chunk size**, so this is purely a
time-versus-memory dial with no accuracy cost. At 8192 DuckDo (21.5 s) matches the
Python package on CPU (22.5 s), which is the real comparison: the two are the same
speed, and the default was giving away a factor of six.
