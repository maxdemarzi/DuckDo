...
| 10 | Frontier | post-1.0 | — | **DONE** — continuous and multi-valued treatments, panel/DiD (plain and doubly robust), synthetic control, longitudinal MSMs, survival (RMST, and time lost under competing risks), mediation, causal discovery, cluster-robust estimation across one-to-many joins, and federated pooling across sites (`do_ate_pool`) landed. All seven items are done; what each leaves open is listed under Phase 10 below |
